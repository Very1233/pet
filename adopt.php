...
<!----> 
<div class="about">
	<h2><a href="adopt_html.php">发布转让</a></h2>
';

$ENDTIME = date("Y-m-d");

$sql_timeout1 = "DELETE FROM LYSQ_INFO WHERE ZR_ID IN (SELECT ZR_ID FROM ZR_INFO WHERE ZR_ENDTIME < date('Y-m-d'))";
 mysql_query($sql_timeout1);

$sql_timeout2 = "DELETE FROM ZR_INFO WHERE ZR_ENDTIME < $ENDTIME";
 mysql_query($sql_timeout2);

$sql = "SELECT USER_ID, PET_PIC, ZR_ID, ZR_REQUEST, ZR_STARTTIME, ZR_ENDTIME, ADDRESS, USER_NAME FROM ZR_INFO 
		LEFT OUTER JOIN USER USING(USER_ID)
		WHERE ZR_INFO.USER_ID=USER.USER_ID  ORDER BY ZR_STARTTIME DESC";
$result = db_query($sql);
    ...
    while($row = db_fetch_array($result)) {
    	echo '<div class="container">
		 ...
		 <div class="about-head" style="outline:8px solid #b28b51;border:8px solid white;margin-left:50px;margin-right:50px">
			 ...
			 <img src="'. $row["PET_PIC"].'" class="img-responsive" alt=""/>
			 <div class="about-info" >
			 <table>
			 <tr>
				 <td width="45%"><p class="p_time">地点：'. $row["ADDRESS"].'</p></td>
				 <td width="30%"><p class="p_time">截止时间：'. $row["ZR_ENDTIME"].'</p></td>
			</tr>
			</table>
				 <p>'. $row["ZR_REQUEST"].'</p>
				 <form action="adopt_req.php" method="post">
							<input type="hidden" name="ly" value="'.$row["ZR_ID"].'">
							<div class="submit-btn">							
									<input type="submit" value="申请领养">							
							</div>
				</form>
			 </div>
